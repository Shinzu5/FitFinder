...
interface ClerkNewMessageModalProps {
  open: boolean;
  onClose: () => void;
...
export function ClerkNewMessageModal({
  ...
  onClose,
  onSelectContact,
}: ClerkNewMessageModalProps) {
  const [query, setQuery] = useState("");
  const searchResults = useClerkMessagesStore((state) => state.searchResults);
  const searching = useClerkMessagesStore((state) => state.searching);
  const searchUsers = useClerkMessagesStore((state) => state.searchUsers);
  const clearSearch = useClerkMessagesStore((state) => state.clearSearch);
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-white/10 bg-[#141414] p-5 shadow-2xl">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-[#0A0A0A] py-2.5 pl-9 pr-3 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-[#FACC15]/50"
          />
        </div>
            ...
            searchResults.map((contact) => (
              ...
              >
                {contact.avatarUrl ? (
                  ...
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#FACC15]/20 text-sm font-semibold text-[#FACC15]">
